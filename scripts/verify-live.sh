...
if [ "$strict" -eq 1 ]; then
  ...
  visual_css_status="$(curl -sS -o "$tmp_visual_css" -w '%{http_code}' --max-time 20 "https://$www/styles.css?v=20260908-minimal-v8")"
  ...
  assert_visual_css_version() {
    page_label="$1"
    page_file="$2"
    grep -q 'styles.css?v=20260908-minimal-v8' "$page_file" || {
