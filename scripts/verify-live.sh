...
if [ "$strict" -eq 1 ]; then
  ...
  visual_css_status="$(curl -sS -o "$tmp_visual_css" -w '%{http_code}' --max-time 20 "https://$www/styles.css?v=20260908-minimal-v11")"
  ...
  sed -n '/^\.site-header,$/,/^}/p' "$tmp_visual_css" | grep -q 'backdrop-filter: none;' || {
    echo "live minimalist CSS asset is missing the flat header surface" >&2
    exit 1
  }
  grep -q 'box-shadow: none;' "$tmp_visual_css" || {
    echo "live minimalist CSS asset is missing the flat surface rule" >&2
    exit 1
  }
  ...
  assert_visual_css_version() {
    page_label="$1"
    page_file="$2"
    grep -q 'styles.css?v=20260908-minimal-v11' "$page_file" || {
  ...
  grep -q 'id="decision-frame"' "$tmp_monero" || {
    echo "live Monero page missing the shared decision frame" >&2
    exit 1
  }
  grep -q 'id="decision-frame"' "$tmp_plasma_page" || {
    echo "live Plasma page missing the shared decision frame" >&2
    exit 1
  }
  ...
  grep -q '06 / Exit + review' "$tmp_monero" || {
    echo "live Monero page missing the exit and review question" >&2
    exit 1
  }
  grep -q '06 / Exit + review' "$tmp_plasma_page" || {
    echo "live Plasma page missing the exit and review question" >&2
    exit 1
  }
  grep -q '06 / Exit + review' "$tmp_aave" || {
    echo "live Aave page missing the exit and review question" >&2
    exit 1
  }
